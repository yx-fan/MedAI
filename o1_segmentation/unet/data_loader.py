...
def get_dataloaders(data_dir="./data/raw", batch_size=2, patch_size=(160, 160, 128), debug=False, random_seed=42):
    ...
    train_transforms = Compose([
        ...
        RandCropByPosNegLabeld(
            ...
            pos=8, neg=1,  # 增加正样本采样，改善类别不平衡（从2恢复到8）
            num_samples=4,  # 保持4个样本，平衡性能和多样性
        ),
        RandFlipd(keys=["image", "label"], prob=0.5, spatial_axis=[0, 1, 2]),
        RandRotate90d(keys=["image", "label"], prob=0.5, max_k=3),
        RandZoomd(keys=["image", "label"], prob=0.3, min_zoom=0.9, max_zoom=1.1, mode="trilinear"),  # 恢复随机缩放
        RandGaussianNoised(keys=["image"], prob=0.2, mean=0.0, std=0.05),  # 恢复噪声，但降低强度避免过度
        RandAdjustContrastd(keys=["image"], prob=0.3, gamma=(0.9, 1.1)),  # 恢复对比度调整，但范围更保守
        RandShiftIntensityd(keys=["image"], prob=0.3, offsets=(-0.05, 0.05)),  # 恢复强度偏移，但范围更保守
