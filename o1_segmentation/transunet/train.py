...
from data_loader import get_transunet_dataloaders
from models import (
    build_model, loss_fn,
...
parser.add_argument("--resume", type=str, default="", help="Path to checkpoint (e.g., data/transunet/transunet_latest.pth)")
parser.add_argument("--extra_epochs", type=int, default=0, help="Additional epochs to train after resume")
...
start_epoch = 0
learning_rate = 2e-4
save_dir = "data/transunet_debug" if args.debug else "data/transunet"
...
# TensorBoard Init
# ==============================
log_dir = os.path.join("tb_logs_transunet", datetime.now().strftime("%Y%m%d-%H%M%S"))
...
# Data Loaders
# ==============================
train_loader, val_loader = get_transunet_dataloaders(
    data_dir="./data/raw",
...
for epoch in trange(start_epoch, num_epochs, desc="Total Progress"):
    ...
    # -------- Save --------
    latest_path = os.path.join(save_dir, "transunet_latest.pth")
    torch.save({
        "epoch": epoch + 1,
    ...
    if fg_dice_mean > best_dice:
        ...
        best_path = os.path.join(save_dir, "transunet_best.pth")
